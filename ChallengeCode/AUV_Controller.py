...
class AUVController():
    ...
    def store(self):
        ...
        self.stored_buoy_green = self.__green_buoys
        self.stored_buoy_red = self.__red_buoys
    ...
    def find_target_and_desired_heading(self, green_buoys, red_buoys, position):
        if green_buoys and red_buoys:
            ...
            print(self.red_buoy_coord)
            print(self.green_buoy_coord)
        elif green_buoys or red_buoys:
    ...
    def decide(self, auv_state, green_buoys, red_buoys, sensor_type = 'donut'):
        self.__heading = auv_state['heading']
        self.__position = np.array(auv_state['position'])
        self.__green_buoys = green_buoys
        self.__red_buoys = red_buoys
        ...
        if self.__speed == None or self.stored_position.all() == None or self.stored_buoy_green == None or self.stored_buoy_red == None or self.stored_heading == None:
            ...
            if self.status == 'SEARCHING FOR BUOYS':
                ...
                print(self.obtuse_check(intersect, self.target, self.__position))
                if intersect == 'Parallel': #doesn't intersect
                    print('intersect is parallel')
                    ...
                    if abs(self.__green_buoys) < abs(self.__red_buoys):
                        temp_target = self.red_buoy_coord
                    ...
                    print('it is an acute angle cute')
                    temp_target = self.green_buoy_coord
                    if abs(self.__green_buoys) < abs(self.__red_buoys):
                        temp_target = self.red_buoy_coord
                    temp_midpoint = np.array([(temp_target[0] + self.__position[0])/2, (temp_target[1] + self.__position[1])/2])
